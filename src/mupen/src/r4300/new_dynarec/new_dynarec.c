	...
	if (strncmp((char *)ROM_HEADER.Name, "GOLDENEYE", 9) == 0) {
		...
		if ((void *)g_rom > (void *)0xffffffff) {
			...
			if (mmap(ROM_COPY, 12582912,
				...
				-1, 0) <= 0) {
				DebugMessage(M64MSG_ERROR, "mmap() failed");
			}
...
static void lsn(u_char hsn[], int i, int *preferred_reg) {
	...
	for (; j >= 0; j--) {
		...
		if (itype[i + j] == INSTR_STORE || itype[i + j] == INSTR_STORELR) {
		...
		if (itype[i + j] == INSTR_STORE || itype[i + j] == INSTR_STORELR || (opcode[i + j] & 0x3b) == 0x39) {
			hsn[INVCP] = j;
		}
	#endif
	...
	if (itype[i] == INSTR_LOAD || itype[i] == INSTR_LOADLR || itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR || itype[i] == C1LS) {
		hsn[TLREG] = 0;
	}
...
static int needed_again(int r, int i) {
	...
	for (j = 0; j < 9; j++) {
		...
		if (itype[i + j] == SYSCALL_ || ((source[i + j] & 0xfc00003f) == 0x0d)) {
			break;
		}
	}
	for (; j >= 1; j--) {
...
static void address_generation(int i, struct regstat *i_regs, signed char entry[]) {
	if (itype[i] == INSTR_LOAD || itype[i] == INSTR_LOADLR || itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR || itype[i] == C1LS) {
		...
		if (itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR) {
		...
		if (ra >= 0) {
			...
			if (rs1[i] == 0) {
				...
				if (rm >= 0) {
					if (!entry || entry[rm] != mgr) {
						if (itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR || opcode[i] == 0x39 || opcode[i] == 0x3D) {
				...
				if (rs1[i] != rt1[i] || itype[i] != INSTR_LOAD) {
					if (!entry || entry[ra] != agr) {
						...
						#ifdef HOST_IMM_ADDR32
							if ((itype[i] != INSTR_LOAD&&opcode[i] != 0x31 && opcode[i] != 0x35) ||
								(using_tlb && ((signed int)constmap[i][rs] + offset) >= (signed int)0xC0000000))
							#endif
							#ifdef RAM_OFFSET
								if ((itype[i] == INSTR_LOAD || opcode[i] == 0x31 || opcode[i] == 0x35) && (signed int)constmap[i][rs] + offset < (signed int)0x80800000)
	...
	if (itype[i + 1] == INSTR_LOAD || itype[i + 1] == INSTR_LOADLR || itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || itype[i + 1] == C1LS) {
		int agr, ra;
		...
		if (ra >= 0) {
			...
			if (c) {
				if (itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || opcode[i + 1] == 0x39 || opcode[i + 1] == 0x3D) {
					// Stores to memory go thru the mapper to detect self-modifying
					// code, loads don't.
		...
		if (ra >= 0) {
			...
			int offset = imm[i + 1];
			int c = (regs[i + 1].wasconst >> rs) & 1;
			if (c && (rs1[i + 1] != rt1[i + 1] || itype[i + 1] != INSTR_LOAD)) {
				...
				#ifdef HOST_IMM_ADDR32
					if ((itype[i + 1] != INSTR_LOAD&&opcode[i + 1] != 0x31 && opcode[i + 1] != 0x35) ||
						(using_tlb && ((signed int)constmap[i + 1][rs] + offset) >= (signed int)0xC0000000))
					#endif
					#ifdef RAM_OFFSET
						if ((itype[i + 1] == INSTR_LOAD || opcode[i + 1] == 0x31 || opcode[i + 1] == 0x35) && (signed int)constmap[i + 1][rs] + offset < (signed int)0x80800000)
	...
	if (i < slen - 1) {
		...
		if (!bt[i + 1]) {
			if (itype[i + 1] == UJUMP || itype[i + 1] == RJUMP || itype[i + 1] == CJUMP || itype[i + 1] == SJUMP) {
				...
				if (itype[i + 2] == INSTR_LOAD&&rs1[i + 2] == reg&&rt1[i + 2] == reg && ((regs[i + 1].wasconst >> hr) & 1)) {
				#ifdef HOST_IMM_ADDR32
			...
			if (itype[i + 1] == INSTR_LOAD&&rs1[i + 1] == reg&&rt1[i + 1] == reg) {
			#ifdef HOST_IMM_ADDR32
...
static void ds_assemble_entry(int i) {
	...
	if (itype[t] == INSTR_LOAD || itype[t] == INSTR_LOADLR || itype[t] == INSTR_STORE || itype[t] == INSTR_STORELR || itype[t] == C1LS)
		load_regs(regs[t].regmap_entry, regs[t].regmap, regs[t].was32, MMREG, ROREG);
	if (itype[t] == INSTR_STORE || itype[t] == INSTR_STORELR || (opcode[t] & 0x3b) == 0x39)
...
static void pagespan_assemble(int i, struct regstat *i_regs) {
	...
	int hr = 0;
	int addr, alt, ntaddr;
	if (i_regs->regmap[HOST_BTREG] < 0) { addr = HOST_BTREG; } else {
		while (hr < HOST_REGS) {
			if (hr != EXCLUDE_REG && hr != HOST_CCREG &&
...
static void pagespan_ds() {
	...
	if (itype[0] == INSTR_LOAD || itype[0] == INSTR_LOADLR || itype[0] == INSTR_STORE || itype[0] == INSTR_STORELR || itype[0] == C1LS)
		load_regs(regs[0].regmap_entry, regs[0].regmap, regs[0].was32, MMREG, ROREG);
	if (itype[0] == INSTR_STORE || itype[0] == INSTR_STORELR || (opcode[0] & 0x3b) == 0x39)
...
void new_dynarec_init() {
	...
	base_addr = VirtualAlloc(NULL, 1 << TARGET_SIZE_2, 0x1000 | 0x2000, 0x40);
#else
#if defined(_MSC_VER)
...
int new_recompile_block(int addr) {
	...
	*/
	...
	for (i = 0; !done; i++) {
		...
		switch (op) {
		case 0x00: strcpy(insn[i], "special"); type = INSTR_NI;
			op2 = source[i] & 0x3f;
			switch (op2) {
			case 0x00: strcpy(insn[i], "SLL"); type = INSTR_SHIFTIMM; break;
			case 0x02: strcpy(insn[i], "SRL"); type = INSTR_SHIFTIMM; break;
			case 0x03: strcpy(insn[i], "SRA"); type = INSTR_SHIFTIMM; break;
			case 0x04: strcpy(insn[i], "SLLV"); type = INSTR_SHIFT; break;
			case 0x06: strcpy(insn[i], "SRLV"); type = INSTR_SHIFT; break;
			case 0x07: strcpy(insn[i], "SRAV"); type = INSTR_SHIFT; break;
			case 0x08: strcpy(insn[i], "JR"); type = RJUMP; break;
			case 0x09: strcpy(insn[i], "JALR"); type = RJUMP; break;
			case 0x0C: strcpy(insn[i], "SYSCALL"); type = SYSCALL_; break;
			case 0x0D: strcpy(insn[i], "BREAK"); type = OTHER; break;
			case 0x0F: strcpy(insn[i], "SYNC"); type = OTHER; break;
			case 0x10: strcpy(insn[i], "MFHI"); type = INSTR_MOV; break;
			case 0x11: strcpy(insn[i], "MTHI"); type = INSTR_MOV; break;
			case 0x12: strcpy(insn[i], "MFLO"); type = INSTR_MOV; break;
			case 0x13: strcpy(insn[i], "MTLO"); type = INSTR_MOV; break;
			case 0x14: strcpy(insn[i], "DSLLV"); type = INSTR_SHIFT; break;
			case 0x16: strcpy(insn[i], "DSRLV"); type = INSTR_SHIFT; break;
			case 0x17: strcpy(insn[i], "DSRAV"); type = INSTR_SHIFT; break;
			case 0x18: strcpy(insn[i], "MULT"); type = INSTR_MULTDIV; break;
			case 0x19: strcpy(insn[i], "MULTU"); type = INSTR_MULTDIV; break;
			case 0x1A: strcpy(insn[i], "DIV"); type = INSTR_MULTDIV; break;
			case 0x1B: strcpy(insn[i], "DIVU"); type = INSTR_MULTDIV; break;
			case 0x1C: strcpy(insn[i], "DMULT"); type = INSTR_MULTDIV; break;
			case 0x1D: strcpy(insn[i], "DMULTU"); type = INSTR_MULTDIV; break;
			case 0x1E: strcpy(insn[i], "DDIV"); type = INSTR_MULTDIV; break;
			case 0x1F: strcpy(insn[i], "DDIVU"); type = INSTR_MULTDIV; break;
			case 0x20: strcpy(insn[i], "ADD"); type = INSTR_ALU; break;
			case 0x21: strcpy(insn[i], "ADDU"); type = INSTR_ALU; break;
			case 0x22: strcpy(insn[i], "SUB"); type = INSTR_ALU; break;
			case 0x23: strcpy(insn[i], "SUBU"); type = INSTR_ALU; break;
			case 0x24: strcpy(insn[i], "AND"); type = INSTR_ALU; break;
			case 0x25: strcpy(insn[i], "OR"); type = INSTR_ALU; break;
			case 0x26: strcpy(insn[i], "XOR"); type = INSTR_ALU; break;
			case 0x27: strcpy(insn[i], "NOR"); type = INSTR_ALU; break;
			case 0x2A: strcpy(insn[i], "SLT"); type = INSTR_ALU; break;
			case 0x2B: strcpy(insn[i], "SLTU"); type = INSTR_ALU; break;
			case 0x2C: strcpy(insn[i], "DADD"); type = INSTR_ALU; break;
			case 0x2D: strcpy(insn[i], "DADDU"); type = INSTR_ALU; break;
			case 0x2E: strcpy(insn[i], "DSUB"); type = INSTR_ALU; break;
			case 0x2F: strcpy(insn[i], "DSUBU"); type = INSTR_ALU; break;
			case 0x30: strcpy(insn[i], "TGE"); type = INSTR_NI; break;
			case 0x31: strcpy(insn[i], "TGEU"); type = INSTR_NI; break;
			case 0x32: strcpy(insn[i], "TLT"); type = INSTR_NI; break;
			case 0x33: strcpy(insn[i], "TLTU"); type = INSTR_NI; break;
			case 0x34: strcpy(insn[i], "TEQ"); type = INSTR_NI; break;
			case 0x36: strcpy(insn[i], "TNE"); type = INSTR_NI; break;
			case 0x38: strcpy(insn[i], "DSLL"); type = INSTR_SHIFTIMM; break;
			case 0x3A: strcpy(insn[i], "DSRL"); type = INSTR_SHIFTIMM; break;
			case 0x3B: strcpy(insn[i], "DSRA"); type = INSTR_SHIFTIMM; break;
			case 0x3C: strcpy(insn[i], "DSLL32"); type = INSTR_SHIFTIMM; break;
			case 0x3E: strcpy(insn[i], "DSRL32"); type = INSTR_SHIFTIMM; break;
			case 0x3F: strcpy(insn[i], "DSRA32"); type = INSTR_SHIFTIMM; break;
			}
			break;
		case 0x01: strcpy(insn[i], "regimm"); type = INSTR_NI;
			...
			case 0x08: strcpy(insn[i], "TGEI"); type = INSTR_NI; break;
			case 0x09: strcpy(insn[i], "TGEIU"); type = INSTR_NI; break;
			case 0x0A: strcpy(insn[i], "TLTI"); type = INSTR_NI; break;
			case 0x0B: strcpy(insn[i], "TLTIU"); type = INSTR_NI; break;
			case 0x0C: strcpy(insn[i], "TEQI"); type = INSTR_NI; break;
			case 0x0E: strcpy(insn[i], "TNEI"); type = INSTR_NI; break;
		...
		case 0x08: strcpy(insn[i], "ADDI"); type = INSTR_IMM16; break;
		case 0x09: strcpy(insn[i], "ADDIU"); type = INSTR_IMM16; break;
		case 0x0A: strcpy(insn[i], "SLTI"); type = INSTR_IMM16; break;
		case 0x0B: strcpy(insn[i], "SLTIU"); type = INSTR_IMM16; break;
		case 0x0C: strcpy(insn[i], "ANDI"); type = INSTR_IMM16; break;
		case 0x0D: strcpy(insn[i], "ORI"); type = INSTR_IMM16; break;
		case 0x0E: strcpy(insn[i], "XORI"); type = INSTR_IMM16; break;
		case 0x0F: strcpy(insn[i], "LUI"); type = INSTR_IMM16; break;
		case 0x10: strcpy(insn[i], "cop0"); type = INSTR_NI;
			op2 = (source[i] >> 21) & 0x1f;
			switch (op2) {
			case 0x00: strcpy(insn[i], "MFC0"); type = COP0; break;
			case 0x04: strcpy(insn[i], "MTC0"); type = COP0; break;
			case 0x10: strcpy(insn[i], "tlb"); type = INSTR_NI;
			...
			break;
		case 0x11: strcpy(insn[i], "cop1"); type = INSTR_NI;
			op2 = (source[i] >> 21) & 0x1f;
			switch (op2) {
			...
			case 0x08: strcpy(insn[i], "BC1"); type = FJUMP;
				...
				break;
			case 0x10: strcpy(insn[i], "C1.S"); type = INSTR_NI;
				switch (source[i] & 0x3f) {
				...
				break;
			case 0x11: strcpy(insn[i], "C1.D"); type = INSTR_NI;
				switch (source[i] & 0x3f) {
		...
		case 0x18: strcpy(insn[i], "DADDI"); type = INSTR_IMM16; break;
		case 0x19: strcpy(insn[i], "DADDIU"); type = INSTR_IMM16; break;
		case 0x1A: strcpy(insn[i], "LDL"); type = INSTR_LOADLR; break;
		case 0x1B: strcpy(insn[i], "LDR"); type = INSTR_LOADLR; break;
		case 0x20: strcpy(insn[i], "LB"); type = INSTR_LOAD; break;
		case 0x21: strcpy(insn[i], "LH"); type = INSTR_LOAD; break;
		case 0x22: strcpy(insn[i], "LWL"); type = INSTR_LOADLR; break;
		case 0x23: strcpy(insn[i], "LW"); type = INSTR_LOAD; break;
		case 0x24: strcpy(insn[i], "LBU"); type = INSTR_LOAD; break;
		case 0x25: strcpy(insn[i], "LHU"); type = INSTR_LOAD; break;
		case 0x26: strcpy(insn[i], "LWR"); type = INSTR_LOADLR; break;
		case 0x27: strcpy(insn[i], "LWU"); type = INSTR_LOAD; break;
		case 0x28: strcpy(insn[i], "SB"); type = INSTR_STORE; break;
		case 0x29: strcpy(insn[i], "SH"); type = INSTR_STORE; break;
		case 0x2A: strcpy(insn[i], "SWL"); type = INSTR_STORELR; break;
		case 0x2B: strcpy(insn[i], "SW"); type = INSTR_STORE; break;
		case 0x2C: strcpy(insn[i], "SDL"); type = INSTR_STORELR; break;
		case 0x2D: strcpy(insn[i], "SDR"); type = INSTR_STORELR; break;
		case 0x2E: strcpy(insn[i], "SWR"); type = INSTR_STORELR; break;
		case 0x2F: strcpy(insn[i], "CACHE"); type = INSTR_NOP; break;
		case 0x30: strcpy(insn[i], "LL"); type = INSTR_NI; break;
		case 0x31: strcpy(insn[i], "LWC1"); type = C1LS; break;
		case 0x34: strcpy(insn[i], "LLD"); type = INSTR_NI; break;
		case 0x35: strcpy(insn[i], "LDC1"); type = C1LS; break;
		case 0x37: strcpy(insn[i], "LD"); type = INSTR_LOAD; break;
		case 0x38: strcpy(insn[i], "SC"); type = INSTR_NI; break;
		case 0x39: strcpy(insn[i], "SWC1"); type = C1LS; break;
		case 0x3C: strcpy(insn[i], "SCD"); type = INSTR_NI; break;
		case 0x3D: strcpy(insn[i], "SDC1"); type = C1LS; break;
		case 0x3F: strcpy(insn[i], "SD"); type = INSTR_STORE; break;
		default: strcpy(insn[i], "???"); type = INSTR_NI; break;
		}
		itype[i] = type;
		opcode2[i] = op2;
	...
	for (i = 0; i < slen; i++) {
		...
		ccadj[i] = cc;
		if (i > 0 && (itype[i - 1] == RJUMP || itype[i - 1] == UJUMP || itype[i - 1] == CJUMP || itype[i - 1] == SJUMP || itype[i - 1] == FJUMP || itype[i] == SYSCALL_)) {
			cc = 0;
		} else {
			cc++;
		}
	...
	for (i = slen - 1; i >= 0; i--) {
		...
		if (itype[i] == RJUMP || itype[i] == UJUMP || itype[i] == CJUMP || itype[i] == SJUMP || itype[i] == FJUMP) {
			...
			for (hr = 0; hr < HOST_REGS; hr++) {
				...
				if (itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || (opcode[i + 1] & 0x3b) == 0x39) {
		...
		for (hr = 0; hr < HOST_REGS; hr++) {
			...
			if (itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR || (opcode[i] & 0x3b) == 0x39) {
				if (regmap_pre[i][hr] == INVCP) nr |= (uint64_t)1 << hr;
				if (regs[i].regmap_entry[hr] == INVCP) nr |= (uint64_t)1 << hr;
			}
			// Don't store a register immediately after writing it,
		...
		for (hr = 0; hr < HOST_REGS; hr++) {
			if (!((nr >> hr) & 1)) {
				...
				if (itype[i] == RJUMP || itype[i] == UJUMP || itype[i] == CJUMP || itype[i] == SJUMP || itype[i] == FJUMP) {
					...
					if (using_tlb) {
						if (itype[i + 1] == INSTR_LOAD || itype[i + 1] == INSTR_LOADLR ||
							itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR ||
							itype[i + 1] == C1LS)
							map = TLREG;
					} else
						if (itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || (opcode[i + 1] & 0x3b) == 0x39) {
					...
					if (itype[i + 1] == INSTR_LOADLR || itype[i + 1] == INSTR_STORELR ||
						itype[i + 1] == C1LS)
						temp = FTEMP;
					if ((regs[i].regmap[hr] & 63) != rs1[i] && (regs[i].regmap[hr] & 63) != rs2[i] &&
						(regs[i].regmap[hr] & 63) != rt1[i] && (regs[i].regmap[hr] & 63) != rt2[i] &&
					...
					if (i > 0) {
						...
						if (using_tlb) {
							if (itype[i] == INSTR_LOAD || itype[i] == INSTR_LOADLR ||
								itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR ||
								itype[i] == C1LS)
								map = TLREG;
						} else if (itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR || (opcode[i] & 0x3b) == 0x39) {
							map = INVCP;
						}
						if (itype[i] == INSTR_LOADLR || itype[i] == INSTR_STORELR ||
							itype[i] == C1LS)
							temp = FTEMP;
	...
	for (i = 0; i < slen - 1; i++) {
		if (itype[i] == UJUMP || itype[i] == CJUMP || itype[i] == SJUMP || itype[i] == FJUMP) {
			if (ba[i] >= start && ba[i] < (start + i * 4))
				if (itype[i + 1] == INSTR_NOP || itype[i + 1] == INSTR_MOV || itype[i + 1] == INSTR_ALU
					|| itype[i + 1] == INSTR_SHIFTIMM || itype[i + 1] == INSTR_IMM16 || itype[i + 1] == INSTR_LOAD
					|| itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || itype[i + 1] == C1LS
					|| itype[i + 1] == INSTR_SHIFT || itype[i + 1] == COP1 || itype[i + 1] == FLOAT
			...
			if (itype[i] != INSTR_STORE&&itype[i] != INSTR_STORELR&&itype[i] != C1LS&&itype[i] != INSTR_SHIFT&&
				itype[i] != INSTR_NOP&&itype[i] != INSTR_MOV&&itype[i] != INSTR_ALU&&itype[i] != INSTR_SHIFTIMM&&
				itype[i] != INSTR_IMM16&&itype[i] != INSTR_LOAD&&itype[i] != COP1&&itype[i] != FLOAT&&
	...
	#endif
	{
		...
		for (i = 0; i < slen - 1; i++) {
			...
			if (itype[i] == UJUMP || itype[i] == RJUMP || (source[i] >> 16) == 0x1000) {
				...
				// Possible match
				if (itype[i] == INSTR_LOAD || itype[i] == INSTR_LOADLR ||
					itype[i] == INSTR_STORE || itype[i] == INSTR_STORELR || itype[i] == C1LS) {
					for (hr = 0; hr < HOST_REGS; hr++) {
						if (hr != EXCLUDE_REG) {
							end[hr] = i - 1;
							for (j = i; j < slen - 1; j++) {
								...
								if (itype[j] == UJUMP || itype[j] == RJUMP || itype[j] == CJUMP || itype[j] == SJUMP || itype[j] == FJUMP) {
									...
									if (itype[j + 1] == INSTR_LOAD || itype[j + 1] == INSTR_LOADLR ||
										itype[j + 1] == INSTR_STORE || itype[j + 1] == INSTR_STORELR || itype[j + 1] == C1LS) {
										score[hr]++;
										end[hr] = j;
									}
								}
								if (itype[j] == UJUMP || itype[j] == RJUMP || (source[j] >> 16) == 0x1000) {
									...
									if (itype[j] == INSTR_LOAD || itype[j] == INSTR_LOADLR ||
										itype[j] == INSTR_STORE || itype[j] == INSTR_STORELR || itype[j] == C1LS) {
										score[hr]++;
										end[hr] = j;
									}
							}
	...
	for (i = 0; i < slen - 1; i++) {
		if (!i || (itype[i - 1] != UJUMP&&itype[i - 1] != CJUMP&&itype[i - 1] != SJUMP&&itype[i - 1] != RJUMP&&itype[i - 1] != FJUMP)) {
			if (!bt[i + 1]) {
				if (itype[i] == INSTR_ALU || itype[i] == INSTR_MOV || itype[i] == INSTR_LOAD || itype[i] == INSTR_SHIFTIMM || itype[i] == INSTR_IMM16 || (itype[i] == COP1&&opcode2[i] < 3)) {
					if (rs1[i + 1]) {
					...
					if (itype[i + 1] == INSTR_LOAD&&rs1[i + 1] && get_reg(regs[i + 1].regmap, rs1[i + 1]) < 0) {
					...
					if (itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || opcode[i + 1] == 0x39 || opcode[i + 1] == 0x3D) { // SB/SH/SW/SD/SWC1/SDC1
					...
					if (itype[i + 1] == INSTR_LOADLR || opcode[i + 1] == 0x31 || opcode[i + 1] == 0x35) { // LWC1/LDC1
						if (get_reg(regs[i + 1].regmap, rs1[i + 1]) < 0) {
							int nr;
					...
					if (itype[i + 1] == INSTR_LOAD || itype[i + 1] == INSTR_LOADLR || itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR/*||itype[i+1]==C1LS*/) {
						if (itype[i + 1] == INSTR_LOAD)
							hr = get_reg(regs[i + 1].regmap, rt1[i + 1]);
						if (itype[i + 1] == INSTR_LOADLR || opcode[i + 1] == 0x31 || opcode[i + 1] == 0x35) // LWC1/LDC1
							hr = get_reg(regs[i + 1].regmap, FTEMP);
						if (itype[i + 1] == INSTR_STORE || itype[i + 1] == INSTR_STORELR || opcode[i + 1] == 0x39 || opcode[i + 1] == 0x3D) { // SWC1/SDC1
