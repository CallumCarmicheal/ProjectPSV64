...
static void do_readstub(int n) {
	...
	int rth, rt;
	int ds;
	if (itype[i] == C1LS || itype[i] == INSTR_LOADLR) {
	...
	assert(rs >= 0);
	if (addr < 0) addr = rt;
	if (addr < 0 && itype[i] != C1LS&&itype[i] != INSTR_LOADLR) addr = get_reg(i_regmap, -1);
	assert(addr >= 0);
	int ftable = 0;
	...
	emit_pusha();
	ds = i_regs != &regs[i];
	int real_rs = (itype[i] == INSTR_LOADLR) ? -1 : get_reg(i_regmap, rs1[i]);
